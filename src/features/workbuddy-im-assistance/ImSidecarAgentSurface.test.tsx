...
describe('ImSidecarAgentSurface', () => {
  ...
  it('uses the shared runtime, hides the context envelope and sends only after teacher approval', async () => {
    ...
    const guide = within(sidecar).getByRole('region', { name: 'TeachBuddy 消息建议' });
    expect(within(guide).getByText('TeachBuddy')).toBeVisible();
    expect(within(guide).getByText('仅你可见')).toBeVisible();
    expect(within(guide).getByText('您好，我会根据当前教学进展，帮您把要发给学生的消息整理好。')).toBeVisible();
    expect(within(guide).getByRole('tablist', { name: '教学阶段' })).toBeVisible();
    expect(within(guide).getByText('高二物理 3 班 · 动量守恒模型 · 在线课堂')).toBeVisible();
    ...
    fireEvent.wheel(conversation, { deltaY: -80 });
    fireEvent.wheel(conversation, { deltaY: 24 });
    expect(within(guide).getByRole('button', { name: '收起 TeachBuddy 消息建议' })).toHaveAttribute('aria-expanded', 'true');
    fireEvent.wheel(conversation, { deltaY: 26 });
    const compactDynamics = within(guide).getByRole('button', { name: '展开 TeachBuddy 消息建议' });
  ...
  it('stops the current task from the composer and continues in the same conversation', async () => {
    ...
    const textbox = within(composer!).getByRole('textbox', { name: '向 TeachBuddy 输入要求' });
    expect(textbox).toBeEnabled();
    await user.type(textbox, '换一种简短说法');
    await user.click(within(composer!).getByRole('button', { name: '发送给 TeachBuddy' }));
