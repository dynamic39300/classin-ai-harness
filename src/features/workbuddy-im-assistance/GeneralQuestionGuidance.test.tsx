...
function setup(responseForRequest: (request: string) => string = () => '接下来8月10日19:00上电磁感应导入。') {
  let session: RuntimeSession = { id: 'general-session', title: '提问', status: 'idle', updatedAt: timestamp, events: [], artifacts: [] };
...
describe('shared IM question guidance', () => {
  ...
  it('starts one real runtime request on click, uses the displayed text and keeps information out of delivery', async () => {
    ...
    const welcome = await screen.findByRole('region', { name: '自由提问引导' }).catch(() => screen.findByLabelText('自由提问引导'));
    ...
    // Wait for health as well as the independently loaded context.
    await waitFor(() => expect(within(welcome).getByText('点一个问题试试，也可以直接输入')).toBeVisible());
    ...
    expect(send.mock.calls[0]![2]).toContain('lesson-induction-0810');
  ...
  it('preserves text, quotes and message drafts, and Escape closes help without changing navigation', async () => {
    const { send, rerender, services } = setup();
    ...
    expect(within(help).getByText('试着问我')).toBeVisible();
