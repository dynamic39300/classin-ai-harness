...
import { activateGeneralQuestion, buildGeneralQuestionRequest, projectGeneralQuestions } from '@domain/workbuddy/general-question-guidance';
...
export function ImSidecarAgentSurface({ services, target, onLocateMessage, onInsertDirectReply, onClose }: Props) {
  ...
  const historyScrollAnchor = useRef<{ height: number; top: number } | null>(null);
  ...
  function revealHistory() {
    ...
    historyScrollAnchor.current = { height: timeline.scrollHeight, top: timeline.scrollTop };
  ...
  const questions = projectGeneralQuestions(catalog?.questionGuidance, hasAnswer);
  const hasHistory = logicalSessions.some(session => session.events.some(event => event.actor === 'teacher' || event.actor === 'agent'));
  const showWelcome = !direct && Boolean(catalog) && !catalogError && !recoveringBinding && historyPresentation.isReady(sessionRef)
    && !runtime.readError && !hasHistory && (!sessionRef || Boolean(runtime.session)) && initialSessionTrail.every(id => logicalSessions.some(session => session.id === id));
  ...
  const questionProtectsDraft = Boolean(composerDraft || imageDrafts.length || reference || running || pending || preparingRequest || draftEditing || (currentDeliveryIntent === 'draft' && lastAgentEvent && acceptedAgentEventId !== lastAgentEvent.id));
  const questionHint = delivery.status === 'sending' ? '消息发送中，请稍候' : running || pending || preparingRequest ? '生成中，点选可准备下一次提问' : questionProtectsDraft || !canSend ? '点选后加入输入框' : '点一个问题试试，也可以直接输入';
  ...
  return (
    <aside className={styles.sidecar} aria-label={`${TEACHBUDDY_IM_ASSISTANT_LABEL}私密协作窗口`} data-dismissible={onClose ? 'true' : 'false'} data-guide-integrated="true" data-question-guidance={!direct ? "true" : "false"} data-surface="floating-assistant" id="workbuddy-im-sidecar">
      ...
      >
        ...
        {showWelcome ? <GeneralQuestionWelcome questions={questions.initial} onQuestion={askQuestion} disabled={delivery.status === 'sending'} hint={questionHint} /> : null}
        {hasHiddenHistory ? <button className={styles.historyEntry} type="button" onClick={revealHistory} aria-label="查看历史消息"><span aria-hidden="true">↑</span> 查看历史<span className={styles.historyHint}>向上滚动也可查看</span></button> : null}
        {visibleSessions.some(({ events }) => events.length) ? <ol className={styles.agentEvents} aria-label={`${TEACHBUDDY_IM_ASSISTANT_LABEL}会话消息`}>{visibleSessions.map((logicalSession) => {
          ...
          return turns.map((turn, turnIndex) => {
            ...
            return <Fragment key={`${logicalSession.id}:${turn.id}`}>{historyPresentation.firstNewTurn?.sessionRef === logicalSession.id && historyPresentation.firstNewTurn.turnId === turn.id ? <li className={styles.newMessageDivider}><span>以下为新消息</span></li> : null}{messages.map((event) => {
        ...
        {visibleSessions.map(session => session.artifacts.filter(isSolutionImage).map(artifact => <SolutionImagePreview key={`${session.id}:${artifact.id}`} artifact={artifact} scope={services.scope} sessionId={session.id} />))}
      ...
      {!direct ? <GeneralQuestionHelp groups={questions.groups} onQuestion={askQuestion} disabled={delivery.status === 'sending'} hint={questionHint} availableHeight={chatHeight} /> : null}
      ...
      <WorkspaceComposer
        ...
        canSubmit={canSend}
        secondaryActions={canStop && sessionRef ? <button type="button" aria-label="停止生成" title="停止生成" disabled={stopping} onClick={() => { setInterruptedSessionRef(sessionRef); void runtime.execute(sessionRef, { kind: 'cancel' }); }}><Square aria-hidden="true" size={14} />停止</button> : undefined}
        value={composerDraft}
      />
      </div>
    </aside>
