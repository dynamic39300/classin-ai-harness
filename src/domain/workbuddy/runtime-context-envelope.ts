...
function shanghaiTime(value: string): string {
  ...
  return `${new Intl.DateTimeFormat('zh-CN', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).format(date)} Asia/Shanghai`;
}
...
export function createRuntimeContextEnvelope(snapshot: BusinessContextSnapshot, teacherRequest: string, visibleTeacherRequest?: string): string {
  const chatContext = snapshot.chatContext ? {
    capturedAt: snapshot.chatContext.capturedAt,
    from: snapshot.chatContext.from,
    complete: snapshot.chatContext.complete,
    omittedCount: snapshot.chatContext.omittedCount,
    referenceId: snapshot.chatContext.referenceId,
    messages: snapshot.chatContext.messages.map(message => ({
      ...(snapshot.chatContext?.referenceId || message.replyToId ? { id: message.id } : {}),
      at: shanghaiTime(message.sentAt),
      author: `${message.authorRole}:${bounded(message.authorName, 40)}`,
      body: bounded(message.body, 4_000),
  ...
  const context = {
    ...
    toolRoute: snapshot.toolRoute,
    sources: snapshot.sources.map(({ kind, sourceRef, permissionScope, capturedAt, freshness, version }) => ({ kind, sourceRef, permissionScope, capturedAt, freshness, version })),
