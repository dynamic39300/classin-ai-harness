...
PRIVATE = ROOT / '.runtime/private/copilot-derived-2026-09-09-v2'
OUTPUT = ROOT / 'src/mocks/scenarios/copilot-contextual'
VERSION = 'im-copilot-contextual-v2'
...
def main(check=False):
    ...
    maps = {'teacher': {sid(teacher['teacher_uid']): 'copilot-teacher-a'}, 'classes': {},
            'students': {}, 'lessons': {}, 'homeworks': {}, 'activities': {}, 'units': {}, 'messages': {}, 'participants': {}, 'coTeachers': {}, 'courses': {}}
    ...
    for class_index, (course, anchor) in enumerate(zip(selected, anchors)):
        ...
        for index, activity in enumerate(sorted(in_class(activities, course), key=lambda row: sid(row['id'])), 1):
            ...
            inventory['activities'].append({'id': anon, 'classId': class_id,
                                            ...
                                            'homeworkId': maps['homeworks'].get(sid(activity['biz_id'])) if activity['biz_type'] == 2 else None,
                                            'lessonId': maps['lessons'].get(sid(activity['biz_id'])) if activity['biz_type'] == 1 else None,
    ...
    # Product class -> course is the LMS category relation, not the English name of DW course_id.
    # Online KB confirms the relation; directory/name access remains unavailable.
    scene_courses = []
    source_course_evidence = []
    for class_index, (class_room, source_class) in enumerate(zip(classes, selected)):
        class_id = class_room['id']
        alpha_id, beta_id = class_id + '-course-alpha', class_id + '-course-beta'
        own_activities = [row for row in in_class(activities, source_class) if not row['is_deleted']]
        categories = {sid(row['category_id']) for row in own_activities}
        if len(categories) != 1 or categories & {'0', '', 'None', 'null'}:
            raise ValueError('Expected one nonempty LMS category reference; cannot invent a source course')
        category = next(iter(categories))
        maps['courses'][category] = alpha_id
        source_course_evidence.append({'classId': class_id, 'courseId': alpha_id,
                                      'categoryReferenceVerified': True, 'catalogObjectFetched': False, 'catalogNameStatus': 'unknown'})
        for course_id, title, relation, mapping in [(alpha_id, '基础巩固课', 'dw-category-derived', 'reference-verified'),
                                                    (beta_id, '综合应用课', 'simulated-course-link', 'unknown')]:
            scene_courses.append({'id': course_id, 'classId': class_id, 'title': title,
                                  'studentIds': list(class_room['studentIds']), 'relationshipSource': relation,
                                  'sourceMappingStatus': mapping, 'catalogNameStatus': 'unknown', 'titleSource': 'independent-alias'})
        for entity_key in ['lessons', 'homeworks', 'events', 'recapMaterials']:
            for entity in pack[entity_key]:
                if entity['classId'] == class_id:
                    entity['courseId'] = alpha_id
        # Inventory retains the actual category association for ALL collected source objects.
        for entity_key, source_id_key, business_type in [('lessons', 'class_id', 1), ('homeworks', 'homework_id', 2)]:
            source_entities = in_class(lessons if entity_key == 'lessons' else homeworks, source_class)
            for source_entity in source_entities:
                matched = [row for row in own_activities if row['biz_type'] == business_type and sid(row['biz_id']) == sid(source_entity[source_id_key])]
                if len(matched) != 1 or sid(matched[0]['category_id']) != category:
                    raise ValueError('Source lesson/homework course relation is missing or ambiguous')
            for entity in inventory[entity_key]:
                if entity['classId'] == class_id:
                    entity['courseId'] = alpha_id
        for entity_key in ['studentWork', 'activities', 'units', 'lessonRoster', 'attendanceSnapshot']:
            for entity in inventory[entity_key]:
                if entity['classId'] == class_id:
                    entity['courseId'] = alpha_id
        # Additional course is independent simulation: shared class/student cohort, separate lesson/homework.
        beta_lesson_id = beta_id + '-simulated-lesson-01'
        beta_homework_id = beta_id + '-simulated-homework-01'
        beta_end = target + 86400 - 20 * 60
        if class_index == 1:
            beta_end -= 2 * 86400  # Same teacher's second class does not overlap the first.
        beta_start = beta_end - 75 * 60
        beta_publish = target + 86400 - 45 * 60
        beta_due = target + 86400 - 10 * 60
        members = class_room['studentIds']
        pack['lessons'].append({'id': beta_lesson_id, 'classId': class_id, 'courseId': beta_id,
                                'title': '综合应用 · 本次课堂', 'teacherId': 'copilot-teacher-a',
                                'startsAt': iso(beta_start), 'scheduledEndsAt': iso(beta_end),
                                'scheduleSource': 'simulated-schedule', 'role': 'anchor',
                                'rosterStudentIds': list(members), 'rosterSource': 'simulated-roster-from-homework-associations'})
        pack['homeworks'].append({'id': beta_homework_id, 'classId': class_id, 'courseId': beta_id,
                                  'title': '阶段巩固练习 1', 'studentIds': list(members),
                                  'publishedAt': iso(beta_publish), 'dueAt': iso(beta_due), 'lessonId': beta_lesson_id,
                                  'lessonLinkSource': 'simulated-link', 'timingSource': 'simulated-events', 'role': 'anchor'})
        event(class_id, beta_start, 'lesson-started', '新增第二产品课程的独立模拟开课，不改写源category关系', courseId=beta_id, lessonId=beta_lesson_id)
        event(class_id, beta_end, 'lesson-ended', '新增第二产品课程的独立模拟结束，与课程甲进度分开', courseId=beta_id, lessonId=beta_lesson_id)
        for student in members:
            event(class_id, beta_start + 60, 'student-entered', '第二课程独立到课事件；同一班级成员不沿用其他课程到课状态',
                  courseId=beta_id, lessonId=beta_lesson_id, studentId=student)
        event(class_id, beta_publish, 'homework-published', '独立模拟第二课程作业；与第一课程同名但业务引用不同',
              courseId=beta_id, homeworkId=beta_homework_id)
        unsubmitted_beta = {members[0], members[-1], members[-2]}
        pending_beta = set(members[1:3])
        for index, student in enumerate(members):
            submitted = beta_due + 60 if student in unsubmitted_beta else beta_publish + 60
            event(class_id, submitted, 'homework-submitted', '同学生在课程甲已交、课程乙未交；提交事件严格按课程作业引用',
                  courseId=beta_id, homeworkId=beta_homework_id, studentId=student)
            graded = beta_due + 120 if student in unsubmitted_beta or student in pending_beta else beta_publish + 120
            event(class_id, graded, 'homework-graded', '第二课程批改状态独立；S6前完成，不改变S1至S8原含义',
                  courseId=beta_id, homeworkId=beta_homework_id, studentId=student)
    pack['courses'] = scene_courses
    # Events created before v2 have an explicit first-course reference; beta events already carry theirs.
    for item in events:
        item.setdefault('courseId', item['classId'] + '-course-alpha')
    pack['events'] = sorted(events, key=lambda item: (item['at'], item['id']))
    pack['checkpoints']['S9'] = iso(target + 86400 - 30 * 60)
    pack['gaps'] += ['second-course-relationship-simulated', 'lms-category-directory-and-name-unavailable']
    inventory['courses'] = [item for item in scene_courses if item['relationshipSource'] == 'dw-category-derived']
    audit = {'version': VERSION, 'snapshotId': manifest['snapshot_id'], 'sourceFilesVerifiedBeforeAndAfter': source_verified,
             ...
             'courseMappingEvidence': source_course_evidence,
             'sourceClassCategoryCoverage': {'classes': len(courses), 'classesWithMultipleActiveCategories': sum(len({sid(row['category_id']) for row in in_class(activities, course) if not row['is_deleted']}) > 1 for course in courses)},
