...
for (const entry of ['messages', 'class-chat']) {
  test(`general questions: ${entry} @a11y`, async ({ page }, testInfo) => {
    ...
    await expect(help.getByText('试着问我')).toHaveCount(0);
    await expect(help.getByRole('button', { name: '关闭提问帮助' })).toBeVisible();
    ...
    await composer.fill('');
    await sidecar.getByRole('button', { name: '可以问什么' }).click();
    await help.getByRole('button', { name: '作业测验' }).click();
    await help.getByRole('button', { name: /还有谁没交/ }).click();
