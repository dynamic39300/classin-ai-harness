...
test('a generated solution image stays in its original turn before later messages', async ({ page }) => {
  const session: RuntimeSession = {
    id: 'solution-image-history-session',
    title: '动量守恒解题图片',
    status: 'idle',
    updatedAt: '2026-09-14T04:00:02.000Z',
    events: [
      {
        id: 'teacher-image-request', runRef: 'solution-image-history-session', sequence: 1,
        occurredAt: '2026-09-14T03:44:23.000Z', updatedAt: '2026-09-14T03:44:23.000Z',
        actor: 'teacher', kind: 'teacher_message', state: 'completed', title: '教师',
        summary: '请生成这道题的解析图片。', objectRefs: [], allowedCommands: [],
      },
      {
        id: 'agent-image-response', runRef: 'solution-image-history-session', sequence: 2,
        occurredAt: '2026-09-14T03:45:54.000Z', updatedAt: '2026-09-14T03:45:54.000Z',
        actor: 'agent', kind: 'process', state: 'completed', title: 'TeachBuddy',
        summary: '解析图片已经生成。', objectRefs: [], allowedCommands: [],
      },
      {
        id: 'teacher-later-message', runRef: 'solution-image-history-session', sequence: 3,
        occurredAt: '2026-09-14T03:59:16.000Z', updatedAt: '2026-09-14T03:59:16.000Z',
        actor: 'teacher', kind: 'teacher_message', state: 'completed', title: '教师',
        summary: '你在线吗？', objectRefs: [], allowedCommands: [],
      },
      {
        id: 'agent-later-response', runRef: 'solution-image-history-session', sequence: 4,
        occurredAt: '2026-09-14T04:00:02.000Z', updatedAt: '2026-09-14T04:00:02.000Z',
        actor: 'agent', kind: 'process', state: 'completed', title: 'TeachBuddy',
        summary: '在线，请继续。', objectRefs: [], allowedCommands: [],
      },
    ],
    artifacts: [{
      id: 'solution-image-artifact', title: '一维动量守恒碰撞速度正负号判定三步法',
      content: '{}', fileRef: 'sf-solution-image', fileName: 'solution.solution.json',
      format: 'json', mediaType: 'application/json', byteSize: 2,
      createdAt: '2026-09-14T03:45:54.500Z', version: 1, status: 'draft',
    }],
  };
  await page.addInitScript(() => {
    window.localStorage.setItem('classin:teachbuddy:im-session-bindings:v1', JSON.stringify({
      'ideal-full|classin-demo-school|teacher-001|class-physics-3': {
        sessionRef: 'solution-image-history-session',
        updatedAt: '2026-09-14T04:00:02.000Z',
      },
    }));
  });
  ...
  const sidecar = page.getByRole('complementary', { name: 'AI 消息助手私密协作窗口' });
  await sidecar.getByRole('button', { name: '查看历史消息' }).click();
  const image = sidecar.getByRole('figure', { name: /一维动量守恒碰撞速度正负号判定三步法/ });
  const laterMessage = sidecar.getByText('你在线吗？', { exact: true });
  await expect(image).toBeVisible();
  await expect(laterMessage).toBeVisible();
  expect(await image.evaluate((node, later) => Boolean(node.compareDocumentPosition(later as Node) & Node.DOCUMENT_POSITION_FOLLOWING), await laterMessage.elementHandle())).toBe(true);
});
...
test('teacher starts a governed recap from Teaching Dynamics without a configuration page', async ({ page }) => {
  ...
  const sidecar = page.getByRole('complementary', { name: 'AI 消息助手私密协作窗口' });
  await expect(sidecar.getByText(/^\d+项$/)).toBeVisible();
  ...
  await expect(guide).toContainText('选一条建议，AI写消息，您确认后发送');
...
test('AI message helper stays expanded while scrolling and toggles from its full header', async ({ page }, testInfo) => {
  ...
  await expect(guide.getByText(/选一条建议，AI写消息，您确认后发送/)).toBeVisible();
...
for (const entry of ['messages', 'class-chat'] as const) {
  test(`structured draft can be sent directly from ${entry}`, async ({ page }, testInfo) => {
    ...
    const sent = page.getByRole('region', { name: '高二物理 3 班会话' })
      .locator('[data-message-body]').filter({ hasText: '当前进度' });
    await expect(sent).toHaveCount(1);
    await expect(sent.getByRole('listitem')).toHaveText([
      '已完成 8 讲', '动量守恒：碰撞模型',
      '8 月 10 日 19:00：电磁感应导入', '8 月 12 日 19:00：楞次定律',
    ]);
    await expect(sent).toContainText('@李明、@周然 @陈晨 @王小明 @张然 @赵可 请带好讲义');
    await expect(sent.locator('[data-message-mention]')).toHaveText(['@所有人', '@李明']);
