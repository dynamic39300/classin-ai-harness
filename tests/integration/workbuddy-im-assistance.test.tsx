...
describe('WorkBuddy IM assistance', () => {
  it('generates, approves, distributes and opens a format-neutral guided explanation', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'AI 消息助手' }));
    const sidecar = screen.getByLabelText('AI 消息助手私密协作窗口');
  ...
  it('keeps WorkBuddy private to teachers in class chat', () => {
    ...
    expect(screen.getByRole('button', { name: 'AI 消息助手' })).toBeInTheDocument();

    unmount();
    renderWorkspace('student-family');
    expect(screen.queryByRole('button', { name: 'AI 消息助手' })).not.toBeInTheDocument();
    expect(screen.queryByLabelText('AI 消息助手私密协作窗口')).not.toBeInTheDocument();
  });
  ...
  it('keeps WorkBuddy persistent across teacher immersive class and direct chats', async () => {
    ...
    expect(within(sidecar).queryByRole('button', { name: '关闭 AI 消息助手' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'AI 消息助手' })).not.toBeInTheDocument();
  ...
  it('closes WorkBuddy after exiting immersive even when it was already open on entry', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'AI 消息助手' }));
    expect(screen.getByLabelText('AI 消息助手私密协作窗口')).toHaveAttribute('data-dismissible', 'true');

    view.rerender(createWorkspaceTree('teacher', undefined, true));
    expect(screen.getByLabelText('AI 消息助手私密协作窗口')).toHaveAttribute('data-dismissible', 'false');

    view.rerender(createWorkspaceTree('teacher'));
    await waitFor(() => expect(screen.queryByLabelText('AI 消息助手私密协作窗口')).not.toBeInTheDocument());
  });
  ...
  it('uses the current teacher direct chat to prepare a reply without sending it', async () => {
    ...
    const trigger = screen.getByRole('button', { name: 'AI 消息助手' });
  ...
  it('closes the private sidecar with Escape and restores the WorkBuddy trigger', async () => {
    ...
    const trigger = screen.getByRole('button', { name: 'AI 消息助手' });
    await user.click(trigger);
    expect(screen.getByLabelText('AI 消息助手私密协作窗口')).toBeInTheDocument();
    await user.keyboard('{Escape}');
    await waitFor(() => expect(screen.queryByLabelText('AI 消息助手私密协作窗口')).not.toBeInTheDocument());
  ...
  it('generates, revises, approves and sends one grouped reminder as the teacher', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'AI 消息助手' }));
    const sidecar = screen.getByLabelText('AI 消息助手私密协作窗口');
    expect(within(sidecar).getByText('AI 消息助手')).toBeInTheDocument();
