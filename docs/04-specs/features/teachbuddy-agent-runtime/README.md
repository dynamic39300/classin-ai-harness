...
- 教师在 TeachBuddy 工作台直接发起文本对话和教学任务，三个教师 Product Profile 使用独立 scope。
- AgentRuntimeAdapter 提供 health/list/create/read/send/cancel/approve；页面不解释供应商原始事件。
- 官方固定版本 Harness Web HTTP 接口由本机 BFF 调用。会话与模型事件映射为现有 ConversationRunEvent；不显示推理内容。
...
密钥只从服务端环境或未提交的 .env 读取。HTTP 服务仅绑定 loopback，校验写请求来源和内容类型；浏览器不能指定任意 Harness 方法、文件路径或工作目录。scope 与 session 的关联由 BFF 持久化并校验。首期模型只接收教师主动输入的文本；不自动下发整个模拟学生库或隐含 Context。三个 profile 的历史和产物相互隔离。当前 Demo 身份不作为生产身份认证。
