...
方案二已把原来分离的`TeachBuddy · 仅你可见`Header 与`教学动态`模块合并为一个顶部协作面。老师第一眼看到身份和“帮您把要发给学生的消息整理好”的用途说明，随后直接使用`课前 / 课中 / 课后 / 总结`四阶段 Tab 和当前消息建议。TeachBuddy 现在作为与左侧聊天工作台等高的完整侧栏：Sidecar 不再缩高，也不再叠加灰色底板、外边框和阴影；全宽顶栏与建议区属于同一连续内容面，不显示重复的`教学动态`标题。
...
- TeachBuddy 顶栏和可展开建议区位于对话滚动容器之外。聊天记录只在下方区域滚动，不会从顶栏上下或两侧露出；顶栏本身不再使用卡片边框和阴影，通过背景色变化区分展开与紧凑状态。
