...
| Copilot Conversation Run UX | [PRD](./COPILOT-CONVERSATION-RUN-UX-PRD.md) · [Feature Spec](./COPILOT-CONVERSATION-RUN-UX-FEATURE-SPEC.md) · [Tickets](./COPILOT-CONVERSATION-RUN-UX-TICKET-BREAKDOWN.md) · [Implementation Review](./COPILOT-CONVERSATION-RUN-UX-IMPLEMENTATION-REVIEW.md) | 如何把多张运行卡收敛为教师消息、单行业务进展、按需处理过程、结果和原位审阅 | `IMPLEMENTED_PENDING_USER_REVIEW` |
