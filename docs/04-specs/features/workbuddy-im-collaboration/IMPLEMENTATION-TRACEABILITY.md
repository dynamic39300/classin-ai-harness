...
| IM-PRD-141—145、148 | Feature Spec §6.19、§7 | IM-031、IM-032、IM-036 | `TeachingDynamicsAdapter`、四阶段 Tab 与准确数量/核对状态、当前阶段默认选择、按线程保存的展开/紧凑呈现与非打断更新 | Domain + Adapter + Component + Browser/Visual | PASS |
...
- Teaching Dynamics Surface：`TeachingDynamics.tsx` 与 `ImSidecarAgentSurface.tsx` 复用消息侧栏和原有 Runtime；身份、引导、阶段与事项位于同一 Surface。事项动作把卡片自然语言 Prompt 原文投影为老师消息，Runtime Context Envelope 只在内部携带能力、对象、格式与证据要求；补问、生成、修订、审阅和发送留在同一逻辑对话，展开状态仅按线程保存在浏览器会话中。
- Continuous Conversation Recovery：`im-agent-session-binding.ts`用有界 Binding Trail 保存当前及先前内部 Runtime Session 引用；Sidecar 按 Trail 聚合事件，让老师通过上滑看到一条连续历史。读取已保存 Binding 返回 404 时透明创建替代 Session 并更新绑定，不呈现新建或历史 Session 控件。
- Runtime Stop Control：`useAgentRuntime`允许停止操作抢占未返回的生成请求并忽略迟到响应；停止失败继续锁定发送，同时保留停止动作供老师重试。
- Teaching Dynamics Verification：Domain、Adapter、Context Envelope、Binding、Runtime、Component 和 Sidecar 共 7 个测试文件、20 项测试，覆盖生命周期、T-1 谨慎语义、自然语言 Prompt 精确投影、内部 Session 历史聚合、stale 404 恢复、原位展开/收起、滚动阈值、停止后续写及停止失败重试；`teachbuddy-im-personalized-services.spec.ts` 的 10 项 Chromium E2E 覆盖消息入口、精确 Prompt、连续历史、透明恢复、恢复失败保留历史、320ms 动效和 axe，收起动效另以 4 worker 并发重复 10 次均通过；1440×900 合并展开态完成实机自查。
