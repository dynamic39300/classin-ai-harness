...
- 默认 IM Copilot 依 D-157 使用固定、可重置的模拟对象，并与 `classin-ai-harness` 2026-09-15 冻结工作树保持用户体验一致；D-155/D-156 形成的真实测试 Module、合同、研究和独立核验页继续保留，但不进入默认消息组合根；生产 API 写回仍在范围外；
